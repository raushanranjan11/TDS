{
	...
	beforeSubmit: function (e) {
		...
		var cruisePackagePrice = g.getData("cruisePackagePrice");
		var cruisePrice = '';
		if (!isNaN(parseFloat(cruisePackagePrice))) {
			cruisePrice = cruisePackagePrice
		} else {
			cruisePrice = (cruisePackagePrice.substring(3, cruisePackagePrice.length).trim());
		}
		...
		var e = {
			...
			data: {
				...
				cruisePackagePrice: cruisePrice,
		...
		if (flag) {
			...
			Ext.Msg.alert("Alert", " Must Link Passengers");
		} else {
			return e
		}
	},
							...
							items: [{
									...
									items: [{
											...
											listeners: {
												render: function () {
													...
													var passengerRange = this.findParentByType('awesomewindow').getData("passengerRange")
														if (passengerRange != '') {
															passengerRange.price = (totalCruisePrice.substring(3, totalCruisePrice.length));
															r.push(passengerRange)
															this.getStore().loadData(r)
														} else {
									...
									items: [{
											...
											items: [{
													...
													listeners: {
														click: function () {
															var a1 = this.findParentByType("awesomewindow").getData("stateRoom");
															...
															var passengerRange = Ext.getCmp("review").getStore().getAt(0).data;
															var adjustmentMarkup = c.getData("adjustmentMarkup")
																var totalCruisePrice = this.findParentByType("awesomewindow").getData("totalCruisePrice");
															...
															TDS.window.setWindow({
																...
																data: {
																	...
																	totalCruisePrice: totalCruisePrice
																},
																dataURI: {
																	pnr: f,
																	offeringURI: b,
